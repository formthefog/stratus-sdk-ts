...
export interface ModelMetrics {
  ...
  // Quality metrics
  embeddingQuality?: number;        // Cosine similarity after compression (%)
  responseQuality?: number;         // Semantic coherence score (0-100)
  ...
  // Cost metrics
  costPer1MTokens?: number;         // USD per 1M tokens
  compressionRatio?: number;        // Storage compression ratio
...
export class ModelComparison {
    ...
    try {
      ...
      if (options.tasks.includes('embeddings')) {
        if (model === 'mjepa-g' && this.mjepaClient) {
          ...
          metrics.compressionRatio = parseFloat(
            this.mjepaClient.getCompressionRatio().replace('x', '')
          );
   ...
   */
  private determineWinners(results: ModelMetrics[]): ComparisonResult['winner'] {
  ...
  generateReport(result: ComparisonResult): string {
    ...
    for (const metrics of result.results) {
      ...
      lines.push(
        `| ${metrics.model} | ${quality} | ${latency} | ${cost} | ${compression} | ${attribution} |`
      );
    }
   ...
   */
  async quickCompare(): Promise<string> {
