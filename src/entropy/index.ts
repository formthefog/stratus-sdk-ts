 ...
 */
function getCodeLengths(root: HuffmanNode, maxValue: number): Uint8Array {
  const lengths = new Uint8Array(maxValue + 1);

  function traverse(node: HuffmanNode, depth: number) {
    if (node.value !== null) {
 ...
 */
function generateCanonicalCodes(codeLengths: Uint8Array): Map<number, { code: number; length: number }> {
  const codes = new Map<number, { code: number; length: number }>();

  // Group symbols by code length
  const symbolsByLength = new Map<number, number[]>();
 ...
 */
export function huffmanEncode(values: Uint8Array): {
  encoded: Uint8Array;
...
export function huffmanDecode(
  ...
  while (decoded.length < count) {
    ...
    if (!found) {
      ...
      throw new Error(
        `Failed to decode at position ${decoded.length}/${count}\n` +
