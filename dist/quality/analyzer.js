...
const ranking_js_1 = require("./ranking.js");
/**
 ...
 */
function analyzeQuality(original, restored, options = {}) {
    ...
    const sampleSize = Math.min(options.sampleSize || original.length, original.length);
    const sampledOriginal = sampleSize < original.length
        ? sampleVectors(original, sampleSize)
        : original;
    const sampledRestored = sampleSize < original.length
        ? sampleVectors(restored, sampleSize)
        : restored;
    // Calculate all metrics
    const metrics = calculateAllMetrics(sampledOriginal, sampledRestored, options);
 ...
 */
function calculateAllMetrics(original, restored, options) {
    // Similarity metrics
    const cosineSims = [];
    const euclideanDists = [];
    const manhattanDists = [];
    const dimensionErrorsList = [];
    for (let i = 0; i < original.length; i++) {
        cosineSims.push((0, metrics_js_1.cosineSimilarity)(original[i], restored[i]));
        euclideanDists.push((0, metrics_js_1.euclideanDistance)(original[i], restored[i]));
        manhattanDists.push((0, metrics_js_1.manhattanDistance)(original[i], restored[i]));
        if (options.includeDimensionAnalysis !== false) {
            const errors = (0, metrics_js_1.dimensionErrors)(original[i], restored[i]);
    ...
    const rankingQueries = Math.min(options.rankingQueries || 100, Math.floor(original.length * 0.1));
    const queries = original.length >= 100
        ? sampleVectors(original, rankingQueries)
        : [];
    const ranking = queries.length > 0
        ? (0, ranking_js_1.calculateRankingMetrics)(queries, original, restored)
        : {
            recallAt10: 1,
            recallAt50: 1,
            recallAt100: 1,
    ...
    const overallQuality = calculateOverallQuality({
        cosineSimilarity: (0, metrics_js_1.calculateStats)(cosineSims),
        ranking,
        distribution,
    });
    // Recommendation
    ...
    return {
        cosineSimilarity: (0, metrics_js_1.calculateStats)(cosineSims),
        euclideanDistance: (0, metrics_js_1.calculateStats)(euclideanDists),
        manhattanDistance: (0, metrics_js_1.calculateStats)(manhattanDists),
 ...
 */
function calculateDistributionMetrics(original, restored) {
    ...
    // Variance ratio
    const origVar = origFlat.reduce((s, v) => s + Math.pow(v - origMean, 2), 0) /
        origFlat.length;
    const restVar = restFlat.reduce((s, v) => s + Math.pow(v - restMean, 2), 0) /
        restFlat.length;
    ...
    const kl = (0, metrics_js_1.klDivergence)(origHist, restHist);
    // Wasserstein distance
    const wasserstein = (0, metrics_js_1.wassersteinDistance)(origFlat, restFlat);
    return {
        klDivergence: kl,
        wasserstein,
        meanShift,
...
function histogram(values, numBins) {
    ...
    for (const val of values) {
        const bin = Math.min(Math.floor((val - min) / binSize), numBins - 1);
        bins[bin]++;
    }
...
function calculateOverallQuality(metrics) {
    ...
    const score = metrics.cosineSimilarity.mean * weights.cosineSimilarity +
...
function getSuggestedLevel(score) {
    if (score >= 0.98)
        return 'Can use High or VeryHigh compression';
    if (score >= 0.95)
        return 'Use Medium compression (current)';
    if (score >= 0.90)
        return 'Use Low compression';
...
function generateWarnings(metrics) {
    ...
    if (metrics.cosineSimilarity.mean < 0.95) {
        warnings.push(`Low cosine similarity (${(metrics.cosineSimilarity.mean * 100).toFixed(1)}%). ` +
            'Consider using a lower compression level.');
    }
    if (metrics.rankingPreservation.recallAt10 < 0.90) {
        warnings.push(`Poor ranking preservation (${(metrics.rankingPreservation.recallAt10 * 100).toFixed(1)}% recall@10). ` +
            'Search quality may be impacted.');
    }
    if (metrics.distributionShift.varianceRatio < 0.8 || metrics.distributionShift.varianceRatio > 1.2) {
        warnings.push(`Significant variance shift (ratio: ${metrics.distributionShift.varianceRatio.toFixed(2)}). ` +
            'Vector distribution has changed.');
    }
    if (metrics.maxDimensionError > 0.1) {
        warnings.push(`High maximum dimension error (${metrics.maxDimensionError.toFixed(4)}). ` +
...
function generateRecommendations(metrics) {
    ...
    if (metrics.recommendation === 'excellent') {
        recommendations.push('Quality is excellent! Consider using a higher compression level to save more space.');
    }
    else if (metrics.recommendation === 'good') {
        recommendations.push('Quality is good. Current compression level is appropriate.');
    }
    else if (metrics.recommendation === 'acceptable') {
        recommendations.push('Quality is acceptable but could be better. Consider using a lower compression level.');
    }
    else {
        recommendations.push('Quality is poor. Use a lower compression level or consider if compression is appropriate for this dataset.');
    }
    if (metrics.cosineSimilarity.stddev > 0.05) {
        recommendations.push('High variance in similarity scores. Quality varies significantly across vectors. ' +
            'Consider analyzing outliers separately.');
    }
    if (metrics.rankingPreservation.recallAt10 > 0.98 && metrics.cosineSimilarity.mean > 0.99) {
        recommendations.push('Excellent ranking preservation! Safe to use in production search systems.');
    }
...
function generateSummary(metrics) {
    ...
    return (`Quality Analysis: ${metrics.recommendation.toUpperCase()} ` +
 ...
 */
function sampleVectors(vectors, n) {
    if (n >= vectors.length)
        return vectors;
    const sampled = [];
