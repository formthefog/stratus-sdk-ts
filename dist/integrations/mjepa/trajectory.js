...
exports.TrajectoryPredictor = void 0;
class TrajectoryPredictor {
    ...
    async predictMany(trajectories, options) {
        ...
        for (let i = 0; i < trajectories.length; i += maxConcurrent) {
            ...
            const batchResults = await Promise.all(batch.map(traj => this.predict({ ...traj, qualityThreshold: options?.qualityThreshold })));
    ...
    findOptimal(trajectories, criteria) {
        if (trajectories.length === 0)
            return null;
        const minQuality = criteria?.minQuality ?? 80;
        const maxSteps = criteria?.maxSteps ?? 10;
        let candidates = trajectories.filter(t => t.summary.qualityScore >= minQuality && t.summary.totalSteps <= maxSteps);
        ...
        if (criteria?.costFunction) {
            ...
            for (const trajectory of candidates) {
                const score = trajectory.predictions.reduce((sum, pred) => sum + criteria.costFunction(pred), 0) /
    ...
    scoreTrajectory(predictions, maxSteps) {
        if (predictions.length === 0)
            return 0;
        const avgConfidence = predictions.reduce((sum, p) => sum + p.action.confidence, 0) / predictions.length;
        const totalProgress = predictions.reduce((sum, p) => sum + p.state_change, 0);
        const normalizedProgress = Math.min(totalProgress * 10, 100);
        const efficiency = predictions.length > 0 ? 100 * (1 - predictions.length / maxSteps) : 0;
        const score = avgConfidence * 100 * 0.4 + normalizedProgress * 0.4 + efficiency * 0.2;
    ...
    compare(trajectories) {
        ...
        const sorted = [...trajectories].sort((a, b) => b.summary.qualityScore - a.summary.qualityScore);
        const avgQuality = trajectories.reduce((sum, t) => sum + t.summary.qualityScore, 0) / trajectories.length;
        const avgSteps = trajectories.reduce((sum, t) => sum + t.summary.totalSteps, 0) / trajectories.length;
        const avgConfidence = trajectories.reduce((sum, t) => {
            const conf = t.predictions.reduce((s, p) => s + p.action.confidence, 0) / t.predictions.length;
            return sum + conf;
