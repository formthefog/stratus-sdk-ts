...
class TrajectoryPredictor {
    ...
    async predictMany(trajectories, options) {
        ...
        for (let i = 0; i < trajectories.length; i += maxConcurrent) {
            ...
            const batchResults = await Promise.all(batch.map(traj => this.predict({
                ...traj,
    ...
    findOptimal(trajectories, criteria) {
        ...
        if (criteria?.costFunction) {
            ...
            for (const trajectory of candidates) {
                const score = trajectory.predictions.reduce((sum, pred) => sum + criteria.costFunction(pred), 0) / trajectory.predictions.length;
                if (score > bestScore) {
                    bestScore = score;
                    best = trajectory;
                }
    ...
    scoreTrajectory(predictions, maxSteps) {
        ...
        const score = (avgConfidence * 100 * 0.4 + // 40% weight on confidence
    ...
    compare(trajectories) {
        ...
        const avgConfidence = trajectories.reduce((sum, t) => {
            const trajConfidence = t.predictions.reduce((s, p) => s + p.action.confidence, 0) / t.predictions.length;
